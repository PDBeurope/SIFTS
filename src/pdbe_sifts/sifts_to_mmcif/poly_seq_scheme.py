...
1. Read the canonical sequence from ``_entity_poly_seq`` (three-letter codes,
   one row per position).  Falls back to ``_entity_poly.pdbx_seq_one_letter_code_can``
...
# Sentinel used for missing auth numbering
_MISSING = "?"
...
def get_canonical_residues(block, entity_id: str) -> list[tuple[int, str]]:
    ...
    *entity_id*, falls back to ``_entity_poly.pdbx_seq_one_letter_code_can``,
