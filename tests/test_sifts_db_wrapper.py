...
class TestSiftsDB:
    ...
    def test_insert_and_query_segment(self, db):
        sdb, conn = db
        sdb.insert_xref_segments([_make_segment()])
        count = conn.execute(
            "SELECT COUNT(*) FROM sifts_xref_segment"
        ).fetchone()[0]
        assert count == 1

    def test_insert_and_query_residue(self, db):
        sdb, conn = db
        sdb.insert_xref_residues([[_make_residue()]])
        count = conn.execute(
            "SELECT COUNT(*) FROM sifts_xref_residue"
        ).fetchone()[0]
        assert count == 1

    def test_clean_segments_removes_entry(self, db):
        sdb, conn = db
        sdb.insert_xref_segments([_make_segment(entry_id="1cbs")])
        sdb.clean_segments(["1cbs"])
        count = conn.execute(
            "SELECT COUNT(*) FROM sifts_xref_segment WHERE entry_id = '1cbs'"
        ).fetchone()[0]
        assert count == 0

    def test_insert_empty_segments_is_noop(self, db):
        sdb, conn = db
        sdb.insert_xref_segments([])
        count = conn.execute(
            "SELECT COUNT(*) FROM sifts_xref_segment"
        ).fetchone()[0]
        assert count == 0

    def test_insert_replaces_existing_entry(self, db):
        sdb, conn = db
        sdb.insert_xref_segments([_make_segment(entry_id="1cbs", id=1)])
        sdb.insert_xref_segments([_make_segment(entry_id="1cbs", id=2)])
        count = conn.execute(
            "SELECT COUNT(*) FROM sifts_xref_segment WHERE entry_id = '1cbs'"
        ).fetchone()[0]
        assert count == 1
